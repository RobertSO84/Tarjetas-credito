...
def index():
    titulo = 'BIENVENIDO - Reporte de Usuarios'
    return render_template('index.html', title = titulo, clientes = base_de_usuarios)

@app.route('/usuario=<nombre>')
def usuario(nombre):
    titulo = 'USUARIOS - Reporte del cliente: {}'.format(nombre)
    indice = 0
    for _ in base_de_usuarios:
        if base_de_usuarios[indice].get_nombre() == nombre:
            val = indice
        indice += 1
    tarjetas_usuarios = base_de_usuarios[val].get_tarjetas()
    return render_template('cliente.html', title = titulo, tarjetas = tarjetas_usuarios, usuario = str(val))
